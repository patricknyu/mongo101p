...
class BlogPostDAO:
    ...
    def add_comment(self, permalink, name, email, body):
	    ...
	    #current_post = get_post_by_permalink(self,permalink)
	    """if current_post is not None:
                comments = []
                comments = current_post['comments']
                comments.append(comment)
"""
            updater = self.posts.update({'permalink':permalink},{'$push':{'comments':comment}})

            return updater['n']  # Change this to return the number of documents updated by the code for HW 3.3
